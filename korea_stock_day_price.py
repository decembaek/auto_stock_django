...
while True:
    ...
    for code in all_code:
        ...
        if stock_price_record_obj_date:
            # 휴장일 및 주말 판단 여부
            _days_ago = (
                datetime.strptime(today, "%Y-%m-%d") - relativedelta(days=minus_days)
            ).strftime("%Y-%m-%d")
            ...
            #     continue
            if stock_price_record_obj_date.strftime("%Y-%m-%d") >= _days_ago:
                # 데이터가 이미 최신이면 다음 종목으로 넘어감
                print(f"Data for {code} is up to date.")
                continue
            ...
            # 100개월 후 계산
            days_later = 100
            target_date = original_date + relativedelta(days=+days_later)
